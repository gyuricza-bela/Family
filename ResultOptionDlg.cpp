...
BEGIN_MESSAGE_MAP( CResultOptionDlg, CDialog )
	//{{AFX_MSG_MAP(CResultOptionDlg)
	ON_BN_CLICKED(IDC_BUTTON_DOWN, OnButtonDown)
	ON_BN_CLICKED(IDC_BUTTON_UP, OnButtonUp)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

CResultOptionDlg::CResultOptionDlg( CAttributeType *pAT, CWnd* pParent )
: CDialog( CResultOptionDlg::IDD, pParent )
{
	//{{AFX_DATA_INIT(CResultOptionDlg)
	//}}AFX_DATA_INIT
	m_pAT = pAT;
}

void CResultOptionDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CResultOptionDlg)
	DDX_Control(pDX, IDC_LIST1, m_lcSelected);
	//}}AFX_DATA_MAP
}

void CResultOptionDlg::OnButtonDown() 
{
...
void CResultOptionDlg::OnButtonUp() 
{
	int iSel = m_lcSelected.GetCurSel();
	if( iSel < 1 )
		return;
...
BOOL CResultOptionDlg::OnInitDialog() 
{
