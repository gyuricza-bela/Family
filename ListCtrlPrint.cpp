...
BOOL CListCtrlPrint::OnPreparePrinting( CPrintInfo* pInfo, BOOL bPrintPreview )
{
	...
	dlg.m_strFtHead  = csFtHead;
	dlg.m_strFtFoot  = csFtFoot;
	dlg.m_strFtPrint = csFtPrint;
	dlg.m_iFtHead    = iFtHead;
	dlg.m_iFtFoot    = iFtFoot;
	dlg.m_iFtPrint   = iFtPrint;
	dlg.m_strHeader  = csPageHead;
	dlg.m_strFoother = csPageFooter;
	dlg.m_strPage    = csPage;

	dlg.m_iSX = rcMarginMM.left;
	dlg.m_iUP = rcMarginMM.top;
	dlg.m_iDX = rcMarginMM.right;
	dlg.m_iDN = rcMarginMM.bottom;

	if( dlg.DoModal() == IDCANCEL )
		return( FALSE );

	csFtHead     = dlg.m_strFtHead;
	csFtFoot     = dlg.m_strFtFoot;
	csFtPrint    = dlg.m_strFtPrint;
	iFtHead      = dlg.m_iFtHead;
	iFtFoot      = dlg.m_iFtFoot;
	iFtPrint     = dlg.m_iFtPrint;
	csPageHead   = dlg.m_strHeader;
	csPageFooter = dlg.m_strFoother;
	csPage       = dlg.m_strPage;

	rcMarginMM = CRect( dlg.m_iSX, dlg.m_iUP, dlg.m_iDX, dlg.m_iDN );
